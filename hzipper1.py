...
This implementation can zip large files, but not unzip them.
"""
import struct
...
def hzip(readName, writeName, url=False):
    ...
    # Encode message into bytes, remember padding of last byte, write that padding, then write the bytes
    encode(text, CB, W) # Encode the text into bytes.
##    W.write(struct.pack('<B',8-pad)) # Write 1byte == the number of bits we are using in the last byte.
##    for byte in encoded:
...
# Make 8 bools into a byte.
def boolsToByte(bools):
    if len(bools) != 8:
        print len(bools)
    byte = 0 # current byte
    for b in bools:
...
# Encode the text string into bytes and write them.
def encode(text, codebook, W):
    bools = []
    for c in text:
        code = codebook[c]
        bools += code
        while len(bools) >= 8:
            towrite = bools[:8]
            bools = bools[8:]
            W.write(struct.pack('<B',boolsToByte(towrite)))
    pad = 8 - len(bools)
    if pad != 0:
        bools += [0]*pad
        W.write(struct.pack('<B',boolsToByte(bools)))
